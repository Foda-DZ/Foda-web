...
import type { ApiOrderStatus, ApiOrder, ApiOrderCustomer } from "../../types/api";
...
> = {
  pending:   { dot: "bg-amber-400",  badge: "bg-amber-50 text-amber-700 border border-amber-200", ring: "ring-amber-400" },
  confirmed: { dot: "bg-blue-400",   badge: "bg-blue-50 text-blue-700 border border-blue-200", ring: "ring-blue-400" },
  shipped:   { dot: "bg-purple-400", badge: "bg-purple-50 text-purple-700 border border-purple-200", ring: "ring-purple-400" },
  delivered: { dot: "bg-green-400",  badge: "bg-green-50 text-green-700 border border-green-200", ring: "ring-green-400" },
  cancelled: { dot: "bg-red-400",    badge: "bg-red-50 text-red-700 border border-red-200", ring: "ring-red-400" },
};
...
function getCustomerName(customerId: string | ApiOrderCustomer): string {
  if (typeof customerId === "object" && customerId?.fullName) return customerId.fullName;
  return "";
}
...
function StatusBadge({ status, label }: { status: ApiOrderStatus; label: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${cfg.badge}`}>
...
export default function OrdersPage() {
  ...
  const tableHeaders = [t.orderId, t.customer, t.date, t.items, t.wilaya, t.total, t.status, t.updateStatus];
      ...
      <div className="p-8 space-y-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-[200px] max-w-sm">
            <SearchIcon
              sx={{ fontSize: 16, position: "absolute", top: "50%", transform: "translateY(-50%)", left: lang === "ar" ? undefined : 12, right: lang === "ar" ? 12 : undefined, color: "rgba(26,26,46,0.3)" }}
            />
            <input
              type="text"
              value={search}
          ...
          {loading ? (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#1A1A2E]/8 bg-[#FAF7F2]">
                  {tableHeaders.map((h) => (
                    <th key={h} className="px-4 py-3 text-start text-[10px] font-semibold tracking-widest uppercase text-[#1A1A2E]/40">
                      {h}
                    </th>
                  ))}
                </tr>
            ...
            <div className="p-16 text-center">
              <div className="w-16 h-16 mx-auto mb-4 bg-[#C9A84C]/8 flex items-center justify-center">
                <ShoppingBagIcon sx={{ fontSize: 28, color: "rgba(201,168,76,0.4)" }} />
              </div>
              <p className="font-display font-bold text-[#1A1A2E] text-base mb-1">
                {t.noOrders} {activeTab !== "all" ? statusLabelMap[activeTab as ApiOrderStatus] : ""}
              </p>
              <p className="text-[#1A1A2E]/40 text-sm max-w-xs mx-auto">
                {activeTab === "all"
                  ? t.noOrdersAll
                  : t.noOrdersStatus.replace("{status}", statusLabelMap[activeTab as ApiOrderStatus])}
              </p>
            </div>
          ) : (
            <table className="w-full text-sm">
                        ...
                        <td className="px-4 py-3.5" onClick={(e) => e.stopPropagation()}>
                          {isUpdating ? (
                            ...
                            <TextField
                              ...
                              sx={{
                                minWidth: 120,
                                "& .MuiOutlinedInput-root": { borderRadius: 0, fontSize: "0.7rem", height: 30 },
                              }}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-[#1A1A2E]/8">
                              ...
                              <div className="md:col-span-1">
                                <p className="text-[10px] font-bold uppercase tracking-widest text-[#1A1A2E]/40 mb-3 flex items-center gap-1.5">
                                  <Inventory2Icon sx={{ fontSize: 11 }} /> {t.orderItems}
                                </p>
                                <div className="space-y-2">
                                  {order.items.map((item, idx) => (
                                    <div key={idx} className="bg-white border border-[#1A1A2E]/8 p-3 transition-all duration-200 hover:shadow-sm">
                                      <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                          ...
                                          <p className="text-[10px] text-[#1A1A2E]/50 mt-1">
                                            {t.qty} {item.quantity} × {item.price.toLocaleString()} {tr.common.dzd}
                                          </p>
                                          {/* Size & Color */}
                                            ...
                                            <div className="flex items-center gap-3 mt-1.5">
                                              {item.selectedChoices.size && (
                                                <span className="inline-flex items-center gap-1 text-[10px] text-[#1A1A2E]/60 bg-[#1A1A2E]/5 px-2 py-0.5">
                                                  <StraightenOutlinedIcon sx={{ fontSize: 10 }} />
                                                  {t.size} {item.selectedChoices.size}
                                                </span>
                                              )}
                                              {item.selectedChoices.color && (
                                                <span className="inline-flex items-center gap-1 text-[10px] text-[#1A1A2E]/60 bg-[#1A1A2E]/5 px-2 py-0.5">
                                                  <PaletteOutlinedIcon sx={{ fontSize: 10 }} />
                                                  {t.color} {item.selectedChoices.color}
                                                </span>
                                              )}
                                            </div>
                                          )}
                              ...
                              <div className="md:col-span-1">
                                <p className="text-[10px] font-bold uppercase tracking-widest text-[#1A1A2E]/40 mb-3 flex items-center gap-1.5">
                                  <PersonOutlineIcon sx={{ fontSize: 11 }} /> {t.customerInfo}
                                </p>
                                <div className="bg-white border border-[#1A1A2E]/8 p-4 space-y-3">
                                  {customerName && (
                                    <div className="flex items-center gap-2">
                                      <PersonOutlineIcon sx={{ fontSize: 13, color: "#C9A84C", flexShrink: 0 }} />
                                      <p className="text-xs font-semibold text-[#1A1A2E]">{customerName}</p>
                                    </div>
                                  )}
                                  {customerEmail && (
                                    <div className="flex items-center gap-2">
                                      <EmailOutlinedIcon sx={{ fontSize: 13, color: "#C9A84C", flexShrink: 0 }} />
                                      <p className="text-xs text-[#1A1A2E]/70">{customerEmail}</p>
                                    </div>
                                  )}
                                  <div className="flex items-center gap-2">
                              ...
                              <div className="md:col-span-1">
                                <p className="text-[10px] font-bold uppercase tracking-widest text-[#1A1A2E]/40 mb-3 flex items-center gap-1.5">
                                  <LocalShippingIcon sx={{ fontSize: 11 }} /> {t.shippingDetails}
                                </p>
                                <div className="bg-white border border-[#1A1A2E]/8 p-4 space-y-3">
                                    ...
                                    <div>
                                      ...
                                      {order.shippingDetails.postalCode && (
                                        <p className="text-[10px] text-[#1A1A2E]/40 mt-0.5">
                                          {t.postal} {order.shippingDetails.postalCode}
                                        </p>
                                      )}
                                    </div>
                                  </div>
                                          ...
                                          <div className="flex flex-col items-center gap-1 flex-1">
                                            <div
                                              className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 ${
                                                isCurrent
                                                  ? cfg.dot + " ring-2 ring-offset-1 " + cfg.ring
                                                  : isPast
                                                    ? cfg.dot
                                                    : "bg-[#1A1A2E]/8"
                                              }`}
